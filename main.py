...
common_params = {
    ...
    "device_model": f"CybroX-UserBot @ {gitrepo.head.commit.hexsha[:7]}",
...
def load_missing_modules():
    ...
    # You'll need to create your own repository for custom modules
    try:
        f = requests.get(
            "https://raw.githubusercontent.com/Rachit-0P/CybroX-Modules/main/full.txt"
        ).text
    except Exception:
    ...
    for module_name in all_modules:
        ...
        if not os.path.exists(module_path) and module_name in modules_dict:
            url = f"https://raw.githubusercontent.com/Rachit-0P/CybroX-Modules/main/{modules_dict[module_name]}.py"
...
async def main():
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("cybroxlogs.txt"), logging.StreamHandler()],
        level=logging.INFO,
    )
    ...
    logging.info("CybroX-UserBot started!")
